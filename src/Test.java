...
public class Test {
    ...
    public static void testIterationCountChangesDistance() {
        ...
        Logger.debug("Low iteration best route:");
        Logger.logTour(resLowIter.bestPosition, tsp, LogLevel.INFO);
        Logger.debug("High iteration best route:");
        Logger.logTour(resHighIter.bestPosition, tsp, LogLevel.INFO);

        Logger.debug("Distance " + resLowIter.bestCost + " after " + lowIters + " iterations.");
        Logger.debug((resLowIter.bestCost / optimalDistance - 1.0) * 100.0 + " percent longer than optimum ("
                + optimalDistance + ").");
        Logger.debug("Distance " + resHighIter.bestCost + " after " + highIters + " iterations.");
        Logger.debug((resHighIter.bestCost / optimalDistance - 1.0) * 100.0 + " percent longer than optimum ("
                + optimalDistance + ").");
